...
test.describe('Grouped controls — spacing and alignment assertions', () => {
  // Desktop gap test removed: with flex-wrap, wide inputs wrap buttons to second row
  // on desktop, making gap measurement invalid. Wrapping tests below cover this case.

  test('horizontal group: items stretch to same height at desktop', async ({
    browser,
  }) => {
    const context = await browser.newContext({
      ...BASE_CONTEXT_OPTIONS,
    ...
    // Fixture 5: mixed control types (scoped by ID)
    const fixture5 = page.locator('#fixture5');
    const select = fixture5.locator('select');
    const input = fixture5.locator('input');
    const button = fixture5.locator('button');
    ...
    log(
      `Fixture 5 — Desktop heights: select=${selectRect.height.toFixed(2)}px, input=${inputRect.height.toFixed(2)}px, button=${buttonRect.height.toFixed(2)}px`
    );

    // Chota existing behavior: select and input have 36px, button has 38px
    // This is pre-existing, not caused by flex-wrap
  ...
  // Gapless desktop test removed: with flex-wrap, items wrap on desktop
  // (wide input fills container), making gap measurement meaningless.
  // Gapless + wrap is tested on narrow viewports (fixtures 4, 8).

  test('gapless group: verify border-radius on first and last items', async ({
    browser,
  }) => {
    const context = await browser.newContext({
      ...BASE_CONTEXT_OPTIONS,
  ...
  test('narrow viewport: horizontal group wraps instead of overflowing (fix #114)', async ({
    browser,
  }) => {
    const context = await browser.newContext({
      ...BASE_CONTEXT_OPTIONS,
    ...
    // With flex-wrap: wrap, items should wrap instead of overflowing
    // scrollWidth should equal container width (no horizontal overflow)
    log(
  ...
  test('narrow viewport: many-item group wraps instead of overflowing (fix #114)', async ({
    browser,
  }) => {
    const context = await browser.newContext({
      ...BASE_CONTEXT_OPTIONS,
    ...
    // With flex-wrap: wrap, 5 buttons should wrap into multiple rows
    // scrollWidth should equal container width (no horizontal overflow)
    log(
  ...
  test('gapless group: verify no horizontal overflow at narrow width (gapless + wrap)', async ({
    browser,
  }) => {
    const context = await browser.newContext({
      ...BASE_CONTEXT_OPTIONS,
  ...
  test('gapless group: verify border-radius preserved on wrapped items', async ({
    browser,
  }) => {
    const context = await browser.newContext({
      ...BASE_CONTEXT_OPTIONS,
